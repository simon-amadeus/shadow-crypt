...
mod tests {
    ...
    fn test_basic_file_encryption() {
        ...
        let result = encrypt_single_file(&input_path, &output_path, password, false);
    ...
    fn test_encryption_with_different_passwords() {
        ...
        let result1 = encrypt_single_file(&input_path, &output1_path, "password1", false);
        let result2 = encrypt_single_file(&input_path, &output2_path, "password2", false);
    ...
    fn test_encryption_preserves_metadata() {
        ...
        // Encrypt file
        let result = encrypt_single_file(&input_path, &output_path, "test_password", false);
        assert!(result.is_ok(), "Encryption failed: {:?}", result.err());
