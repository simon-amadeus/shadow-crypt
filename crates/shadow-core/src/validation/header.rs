...
// File header validation functions
// All code related to validating file headers lives here

use crate::types::{FileHeader, FilenameData};
use crate::errors::ValidationError;
use crate::constants::{
    MAGIC_BYTES, ALGORITHM_ID_XCHACHA20_POLY1305,
...
pub fn validate_header_bytes(data: &[u8]) -> Result<(), ValidationError> {
    ...
    if data.len() >= 8 {
        ...
        if !constant_time_eq(magic_slice, MAGIC_BYTES) {
            return Err(ValidationError::InvalidMagic {
                expected: *MAGIC_BYTES,
                actual: {
...
fn validate_filename_data_consistency(
    ...
    match (filename_data, obfuscation_flag) {
        (FilenameData::Plaintext(_), OBFUSCATION_FLAG_DISABLED) => Ok(()),
        (FilenameData::Encrypted { .. }, OBFUSCATION_FLAG_ENABLED) => Ok(()),
...
mod tests {
    ...
    fn create_valid_header() -> FileHeader {
        FileHeader {
            ...
            obfuscation_flag: OBFUSCATION_FLAG_DISABLED,
            content_hash: [0u8; 32],
    ...
    fn test_validate_file_header_inconsistent_filename_data() {
        let mut header = create_valid_header();
        header.obfuscation_flag = OBFUSCATION_FLAG_ENABLED; // But filename_data is plaintext
        let result = validate_file_header(&header);
    ...
    fn test_validate_filename_data_consistency_plaintext_disabled() {
        ...
        let result = validate_filename_data_consistency(&filename_data, OBFUSCATION_FLAG_DISABLED);
        assert!(result.is_ok());
    }
    ...
    fn test_validate_filename_data_consistency_encrypted_enabled() {
        ...
        let result = validate_filename_data_consistency(&filename_data, OBFUSCATION_FLAG_ENABLED);
        assert!(result.is_ok());
    }
