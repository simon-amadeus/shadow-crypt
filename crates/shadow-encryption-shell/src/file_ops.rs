...
use std::fs;
use shadow_core::{SecureString, hash_content, serialize_header, SerializationError};
use shadow_shell::{ShellError, read_file_safely, write_file_atomically};
use shadow_encryption_core::{EncryptedFile, encrypt_file};
use shadow_encryption_core::pipeline::{create_encryption_request, EncryptionError};
...
pub fn write_encrypted_file(encrypted: &EncryptedFile, output_path: &Path) -> Result<(), EncryptionFileError> {
...
fn read_shadow_file_content_hash(path: &Path) -> Result<[u8; 32], EncryptionFileError> {
    ...
    if bytes_read < 42 { // 10 + 32 for hash
        return Err(EncryptionFileError::InvalidShadowFile(path.to_path_buf()));
    }
...
mod tests {
    ...
    fn test_output_exists_no_force() {
        ...
        assert!(matches!(result.unwrap_err(), EncryptionFileError::OutputExists(_)));
    }
