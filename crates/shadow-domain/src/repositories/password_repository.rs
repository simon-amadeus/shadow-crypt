...
impl PasswordRepository for MockPasswordRepository {
    ...
    fn validate_password_strength(&self, password: &str) -> PasswordStrength {
        // TODO: Move password validation logic to domain or create abstraction
        // For now, provide simple validation
        if password.len() >= 8 {
            PasswordStrength::Strong
        } else {
            PasswordStrength::Weak { issues: vec!["Password too short".to_string()] }
        }
    }
}