//! File encryption implementation (V3-only)
//! 
//! This module provides file encryption functionality using XChaCha20-Poly1305
//! with V3 TLV-based metadata storage.

mod encrypt_file_v3;

// Re-export the clean V3 implementation
pub use encrypt_file_v3::encrypt_single_file_v3 as encrypt_single_file;
...
/// 
/// # Security
/// * Uses XChaCha20-Poly1305 authenticated encryption by default
/// * Derives keys using Argon2id with secure parameters
/// * Stores original metadata securely in V3 TLV fields
...
pub fn encrypt_single_file(
    ...
    let actual_output_path = if obfuscate_filename {
        // Note: Need to update this function to handle new key format
        generate_obfuscated_output_path(input_path, output_path, &master_key)?
    } else {
...
pub fn encrypt_single_file_with_params(
    ...
    // Add encrypted metadata to TLV fields
    header.tlv_fields.add_field(TlvFieldType::FileMetadata, encrypted_metadata.clone());
    ...
    if let Some(filename) = input_path.file_name()
        && let Some(filename_str) = filename.to_str() {
            ...
            // Add encrypted filename to TLV fields
            header.tlv_fields.add_field(TlvFieldType::OriginalFilename, encrypted_filename.clone());
        }
    ...
    if let Some(parent) = input_path.parent()
        && let Some(parent_str) = parent.to_str() {
            ...
            // Add encrypted directory path to TLV fields
            header.tlv_fields.add_field(TlvFieldType::DirectoryPath, encrypted_path.clone());
        }
    ...
    // V1 filename authentication system removed - V3 uses integrated TLV authentication
